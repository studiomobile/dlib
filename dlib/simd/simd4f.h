...
#elif defined(DLIB_HAVE_NEON)
        //http://stackoverflow.com/questions/6759897/how-to-divide-in-neon-intrinsics-by-a-float-number/6822407#6822407
...
#elif defined(DLIB_HAVE_NEON)
        auto result = vmvnq_s32(vceqq_f32(lhs, rhs));
        return result;
#else
        return simd4f_bool(lhs[0]!=rhs[0],
                           lhs[1]!=rhs[1],
...
#elif defined(DLIB_HAVE_NEON)
        auto prod = vmulq_f32(lhs, rhs);
        return sum(simd4f(prod));
#else
        return sum(lhs*rhs);
#endif
    }
...
#elif defined(DLIB_HAVE_NEON)
        //vrsqrteq_f32 gives initial estimate (d)
        //Then apply Newton method two times
        //x[n+1] = x[n]*(3 - d*x[n]^2)/2
        auto x0 = vrsqrteq_f32(item);
        auto x1 = vmulq_f32(x0, vrsqrtsq_f32(vmulq_f32(x0, x0), item));
        auto x2 = vmulq_f32(x1, vrsqrtsq_f32(vmulq_f32(x1, x1), item));
        auto result = vmulq_f32(item, x2);
        return result;
#else
